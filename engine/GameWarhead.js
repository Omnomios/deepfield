define(['ExtraMath','ClientEffect','GameSprite','GameState'],function(ExtraMath, ClientEffect, GameSprite, GameState)
{
	function GameWarhead(type)
	{
		...
		function launch()
		{
			if(this.stat == undefined) return false;
		...
		function adjust()
		{
			...
			{
				case "projectile":
					var offs = ExtraMath.rotatePoints(0, -(this.stat.speed*global.maintimer.delta), this.rot);
					this.pos.x += offs.x;
					this.pos.y += offs.y;
					this.set_qt();
				break;
				...
				case "guided":
					...
					if(target == null)
					{
						...
						if(ailist.length > 0)
						{
							...
							for(var i in ailist)
							{
								if(this.valid_faction(GameState.unit[ailist[i].id].faction))
								{
			...
			if(ExtraMath.distance(this.launchpos,this.pos) > 0.2)
			{
				if(this.qt.list.length > 0)
				{
					...
					{
						if(this.valid_faction(GameState.unit[this.qt.list[i].id].faction) && this.ignore != this.qt.list[i].id)
						{
							var offs = ExtraMath.rotatePoints(0, -(this.stat.speed*global.maintimer.delta), this.rot);
		...
		function testhit()
		{
			...
			for(var i in this.qt.list)
			{
				if(GameState.unit[this.qt.list[i].id] == undefined)
					continue;
