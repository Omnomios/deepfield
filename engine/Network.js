...
define(function()
{
	...
	var Network_Server =
	{
		...
		{
			for(i in this.connection.socket)
			{
				this.send(message,i);
			}
		},
		...
		accept: function accept(ws)
		{
			...
			while(this.connection.socket[pointer] != undefined && this.connection.socket[pointer] != null)
			{
				pointer++;
			...
			if(this.connection.socket[pointer] != undefined)
			{
